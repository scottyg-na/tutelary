...
const MEETINGS_API_PARAMS = '?switcher=GetSearchResults&data_field_key=duration_time,start_time,weekday_tinyint,service_body_bigint,longitude,latitude,location_province,location_municipality,location_info,location_text,formats,format_shared_id_list,comments,meeting_name,id_bigint,meeting_name,location_text,formatted_comments&recursive=0&sort_keys=start_time';

export const getMeetingsFromVirtualNA = async (timezone: string = 'GMT') => {
    ...
    const meetings = responseJson.map(m => {
        ...
        meeting.sourceService = parseInt(service_body_bigint, 10);
